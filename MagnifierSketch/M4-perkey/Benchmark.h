#include <bits/stdc++.h>
#include <sys/time.h>
#include "include/framework/andor/andor_sketch.hpp"
...
public:
	...
	std::pair<std::pair<double, double>, double> Run(double w, uint32_t memory, uint32_t compression) {
        ...
        //sketch::AndorSketch<TDigest>* td = new AndorSketch<TDigest>(memory, 2, 0);
        AndorSketch<TDigest> td(memory, 2, 0);
        //AndorSketch<mReqSketch> td(memory, 2, 0);
        ...
        for (int i = 0; i < 20000000; i++)
        {
            td.append(ins[i].first, ins[i].second);
        }
        ...
        for (int i = 0; i < num; i++)
        {
            ans[i] = td.quantile(qrys[i],query_quantile);
            //std::cout << "ans: " << ans[i] << std::endl;
            //ans[i] = td.query(qrys[i], query_quantile); （id，w）-> value
        }
...
public:
	...
	zipf_Benchmark(std::string path) {
        ...
        uint64_t dataset = static_cast<uint64_t>(reinterpret_cast<uintptr_t>(dataset));
	...
	std::pair<std::pair<double, double>, double> Run(double w, uint32_t memory, uint32_t compression) {
        ...
        for (int i = 0; i < num; i++)
        {
            ...
            ans[i] = td.quantile(qrys[i],query_quantile);//(w,id)->value
            //std::cout << "ans: " << ans[i] << std::endl;
            //std::cout << "qrys: " << qrys[i] << std::endl;
        }
        gettimeofday( &t_end, NULL );
        ...
        for (int i = 0; i < num; i++) {
            ...
            error_qs += fabs(predict_quantile_qs - query_quantile);
            //std::cout <<"query_quantile:" << query_quantile << "\n";
            //std::cout <<"predict_quantile_qs:" << predict_quantile_qs << "\n";
        }
...
public:
	...
	Tap_Benchmark(std::string path) {
        ...
        //std::cout<< load_result.length << std::endl;
	}
	~Tap_Benchmark() {}
    void Init()
    {
