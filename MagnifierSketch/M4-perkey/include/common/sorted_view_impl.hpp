...
namespace sketch {
    SortedView::SortedView(u64 num) : totalWeight(0) {
        view.reserve(num);
    }

    void SortedView::insert(u64_const_iter first, u64_const_iter last,
                             u64 weight) {
    ...
    void SortedView::insert(u64 value, u64 weight) {
    ...
    u64 SortedView::rank(u64 item, bool inclusive) const {
        if (view.empty()) {
            throw std::runtime_error("rank on empty view");
        }

        u64 rk = vec_rank(view, {item, 0}, inclusive, witem::value_less);
        return rk == 0 ? 0 : view[rk - 1].weight;
    }

    f64 SortedView::nomRank(u64 item, bool inclusive) const {
        f64 rk = rank(item, inclusive);
        return rk / totalWeight;   // normalize
    }

    u64 SortedView::quantile(f64 nom_rank, bool inclusive) const {
        if (view.empty()) {
            throw std::runtime_error("get quantile on empty view");
        }

        f64 tmp = nom_rank * totalWeight;
        u64 weight = inclusive ? std::ceil(tmp) : tmp;

        u64 rk = vec_rank(view, {0, weight}, !inclusive, witem::weight_less);
    ...
    SortedView::operator sketch::Histogram() const {
        ...
        vec_u64 height(vec.size() - 1, 0);
        
        for (u64 i = 0; i < vec.size(); ++i) {
            split[i] = vec[i].value;
        }

        for (u64 i = 0; i + 1 < vec.size(); ++i) {
