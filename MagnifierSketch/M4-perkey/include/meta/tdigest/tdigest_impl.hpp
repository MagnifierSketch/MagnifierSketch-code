...
namespace sketch{
    ...
    u64 TDigest::memory() const {
        const u64 counter_bits = std::ceil(std::log2(static_cast<f64>(cap) + 1));
        const u64 centroid_bits = counter_bits + 32;
    ...
    void TDigest::append(u64 item) {
        ...
        if (pos == nullptr) {
            vec_insert_ordered(centroids, Centroid(item, 1),
                               Centroid::mean_less);
            max_weight = std::max(max_weight, static_cast<sketch::u64>(1u));
        } else {
            pos->append(item);
    ...
    Centroid* TDigest::findAppendPos(u64 item) {
        Centroid* pos = nullptr;
        f64 min_dist = UINT64_MAX;
        u64 max_weight = 0;
    ...
    u64 TDigest::quantile(f64 nom_rank) const {
        if (empty()) {
