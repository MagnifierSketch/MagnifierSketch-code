...
public:
	...
	std::pair<std::pair<double, double>, double> Run(uint32_t memory, double eps, double query_w) 
    {
        ...
        for(int j=0;j<100;j++){
            ...
            gettimeofday( &t_end, NULL );
            sum += ((double)(t_end.tv_sec - t_start.tv_sec) + (double)(t_end.tv_usec - t_start.tv_usec) / 1000000.00);
        }
        std::cout << num << "\n";
        double totaltime2 = sum/100.00;
        double throughput2 = double(num) / totaltime2;
...
public:
	...
	std::pair<std::pair<double, double>, double> Run(uint32_t memory, double eps, double query_w) 
    {
        ...
        compare_gk<uint64_t>* gk_sketch = new  compare_gk<uint64_t>(memory,eps,14000000);
